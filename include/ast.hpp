...
public:
    ...
    Agnode_t *draw(Agraph_t *g) const override {
        ...
        agsafeset(node, const_cast<char *>("label"), const_cast<char *>(label.c_str()),
                  const_cast<char *>(""));
        agsafeset(node, const_cast<char *>("shape"), const_cast<char *>("ellipse"),
                  const_cast<char *>(""));
        return node;
    }
};

enum struct BinOp { ADD, SUB, MUL, DIV };
...
public:
    ...
    Agnode_t *draw(Agraph_t *g) const override {
        ...
        agsafeset(node, const_cast<char *>("label"), const_cast<char *>(op_str.c_str()),
                  const_cast<char *>(""));
        agsafeset(node, const_cast<char *>("shape"), const_cast<char *>("circle"),
                  const_cast<char *>(""));
